...
export default function InvitationsList({
  ...
  useEffect(() => {
    getInvitations(session)
  }, [session])

  async function getInvitations(session: SessionResponse) {
    const response = await fetch(`/api/users/${session.user.id}/invitations`, {
      method: "GET",
      headers: {
  ...
  async function reply(choice: boolean, invitationId: string) {
    const response = await fetch(`/api/users/${session.user.id}/invitations/${invitationId}/reply`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
    ...
    <div className="flex flex-col gap-2 w-full">
      ...
      {invitations.map((invitation) => (
        <div key={invitation.id} className="flex items-center gap-2 w-full px-4 py-1 border rounded-md border-neutral-700 justify-between">
          <p><b>{invitation.createdBy.name}</b> invite you to join <b>{invitation.folder.name}</b></p>
          <div className="flex gap-2">
            <button onClick={() => reply(true, invitation.id)}>
            ...
            <button onClick={() => reply(false, invitation.id)}>
