...
import InvitationsPopup from "@/components/InvitationsPopup"
...
export default function InvitationsPopupButton({
  ...
  return (
    <>
      <button type="button" onClick={() => setIsNotificationPopupVisible(true)}>
      ...
      {isNotificationPopupVisible &&
        createPortal(
          <InvitationsPopup onClose={() => setIsNotificationPopupVisible(false)} session={session} />,
          document.body
        )
      }
    </>
